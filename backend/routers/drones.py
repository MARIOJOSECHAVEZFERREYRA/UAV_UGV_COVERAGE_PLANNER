...
@router.get("/")
def get_list_drones(db: Session = Depends(get_db)):
...
def get_drone_default_mission_config(name: str, db: Session = Depends(get_db)):
    """
...
def get_drone(name: str, db: Session = Depends(get_db)):
    d = _drone_or_404(name, db)
    mass_takeoff_max_kg = d.mass_empty_kg + d.mass_battery_kg + d.mass_tank_full_kg
